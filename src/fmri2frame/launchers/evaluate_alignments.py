...
def eval_brain_decoder_wrapper(args):
    ...
    print(
        f"Evaluate decoder {reference_subject} {leftout_subject} {latent_type} "
        f"div={divergence} alpha={alpha} rho={rho} eps={eps} reg_mode={reg_mode}"
    )

    macaques_alignments_path = exps_path / "alignments" / (
